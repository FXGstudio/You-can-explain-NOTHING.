...
import net.mcbbs.youcanexplainnothing.block.PaperBlockBlock;
import net.mcbbs.youcanexplainnothing.block.GumLogBlock;
import net.mcbbs.youcanexplainnothing.block.GumLeavesBlock;
...
public class LinneaBiomeBiome extends YouCanExplainNothingModElements.ModElement {
	@ObjectHolder("you_can_explain_nothing:linnea_biome")
	public static final CustomBiome biome = null;
	public LinneaBiomeBiome(YouCanExplainNothingModElements instance) {
		super(instance, 86);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(PaperBlockBlock.block.getDefaultState(),
							PaperBlockBlock.block.getDefaultState(), PaperBlockBlock.block.getDefaultState())));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == PaperBlockBlock.block.getDefaultState().getBlock()
							|| ground == PaperBlockBlock.block.getDefaultState().getBlock())
							&& (ground2 == PaperBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == PaperBlockBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), PaperBlockBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == GumLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, GumLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, GumLogBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												GumLeavesBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == GumLogBlock.block.getDefaultState().getBlock()
					|| blockType == GumLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == PaperBlockBlock.block.getDefaultState().getBlock()
					|| blockType == PaperBlockBlock.block.getDefaultState().getBlock();
		}
